"""
Module to read and interpolate zonal section data from OGCMs.

This module provides a class {ZonalSections} to read and interpolate
zonal section data from netCDF files generated by OGCMs. The module
also provides a function {interpolate} to interpolate data from one
section onto the coordinates of another section.
...
class ZonalSections(object):
    ...
    ** Assumptions **
    ...
    """
    def __init__(self, file, config, section):
        ...
        if config.has_option(section, 'maskf'):
            ...
            print("{}: using mask information from {}.".format(self.var, self.file))
        else:
    ...
    def _read_mask(self):
        ...
        if maskvar.ndim == 4: # If time dim present in mask field
            ...
            mask = maskvar[:,self.j1:self.j2+1, self.i1:self.i2+1] == self.maskmdi
        # Return mask data as ndarray:
        return mask.values
    ...
    def _extract_section(self, ds):
        # Read variable data depending on whether 2D surface field or 3D field:
        if self.surface_field:
            # Extract 2D surface field data [time, y, x]:
    ...
    def _read_data(self):
        ...
        if self.surface_field:
            # Extract 2D surface field coordinates:
            self.x = data[self.xcoord].values
            self.y = data[self.ycoord].values
            # Extract time coordinate:
            self.dates = data[self.tcoord].values
        else:
            # Extract 3D field coordinates:
            self.x = data[self.xcoord].values
            self.y = data[self.ycoord].values
            self.z = data[self.zcoord].values
            # Extract time coordinate:
            self.dates = data[self.tcoord].values
