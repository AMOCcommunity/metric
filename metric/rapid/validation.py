...
def _plot_fc_timeseries(
    ...
    axs[1].set_ylim([np.min([t_fc_fwt.min(), ds_obs_oht.T_fc_fwt.min()]) - 0.75,
                     np.max([t_fc_fwt.max(), ds_obs_oht.T_fc_fwt.max()]) + 0.75])
...
def _plot_rapid_oht_components(
    ...
    axs[0].set_ylim([np.min([q_sum.min(), q_ek.min(), q_fc.min(), q_wbw.min(), q_geoint.min(), q_eddy.min()]) - 0.7,
    ...
    if ds_obs is not None:
        ...
        axs[1].set_xlabel('Time', fontsize=12, fontweight='bold')
        axs[1].set_ylim([(np.min([ds_obs.Q_sum.min(), ds_obs.Q_ek.min(), ds_obs.Q_fc.min(), ds_obs.Q_wedge.min(), ds_obs.Q_int.min(), ds_obs.Q_eddy.min()]) / 1E15) - 0.7,
...
def make_rapid_validation_report(
    model_ds: xr.Dataset,
    model_name: str,
    output_pdf: str,
    ensemble: bool = False,
    ...
    with RAPID-MOCHA observations.

    Parameters
    ----------
    model_ds : xr.Dataset
        Model dataset containing METRIC RAPID 26.5N AMOC diagnostics.
    model_name : str
        Name of the model.
    output_pdf : str
    ...
    pdf.ln(5)
    plot_file = "./figures/metric_moc_streamfunctions_26N.jpg"
    ...
    pdf.ln(5)
    plot_file = "./figures/metric_moc_components_26N.jpg"
    ...
    pdf.ln(5)
    plot_file = "./figures/metric_fc_timeseries_26N.jpg"
    ...
    pdf.ln(5)
    plot_file = "./figures/metric_oht_components_26N.jpg"
    ...
    pdf.ln(5)
    plot_file = "./figures/metric_oht_geometric_26N.jpg"
    ...
    pdf.ln(5)
    plot_file = "./figures/metric_oft_components_26N.jpg"
    ...
    pdf.ln(5)
    plot_file = "./figures/metric_amoc_vs_oht_26N.jpg"
