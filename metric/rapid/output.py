...
def create_netcdf(config, rapid_trans, model_trans, fc_trans,
    ...
    if isinstance(rapid_trans.dates[0], cftime.datetime):
      time.calendar = utils.get_cftime_calendar(rapid_trans.dates)
      timestamp = date2num(rapid_trans.dates, units=time.units, calendar=time.calendar)
    elif isinstance(rapid_trans.dates[0], np.datetime64):
      time.calendar = 'gregorian'
      timestamp = ((rapid_trans.dates - np.datetime64('1900-01-01T00:00:00'))
                  / np.timedelta64(1, 's'))
    else:
      raise TypeError(f"Invalid type ({type(rapid_trans.dates[0])}) for {tdim.name} coordinate. Expected cftime.datetime or np.datetime64.")
